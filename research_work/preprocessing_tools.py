...
class RatingTransformer(BaseEstimator, TransformerMixin):
    def __init__(self, review_col='Rating'):
    ...
    def transform(self, data):
        new_data = data.copy()
        new_data.loc[new_data[self.review_col] >= 4, 'Label'] = 2
        new_data.loc[new_data[self.review_col] == 3, 'Label'] = 1
        new_data.loc[new_data[self.review_col] <=2, 'Label'] = 0
        return new_data
...
class Normalizer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, data):
        ...
        new_data['temp_col'] = new_data['Review'].apply(lambda x: len(x))
